...
def schema_hint_for_prompt(allowed_tables: list) -> str:
    ...
    lines = [
        ...
        "via a real 'Invalid column name' error. The confirmed table for TAT is trnparamresult "
        "(BILLDATE = sample/order time, CREATEDATE = result time — both confirmed real columns) — "
        "go straight there for any TAT question, don't rely on schema search alone to surface it, "
        "and don't try trninvlabdet.CREATEDATE, it doesn't exist. Two things are BOTH required every "
