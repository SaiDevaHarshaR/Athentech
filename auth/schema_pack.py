...
def schema_hint_for_prompt(allowed_tables: list) -> str:
    ...
    lines = [
        ...
        "of mstdepartment's 9 broad names, resolve through mstsubdepartment FIRST — use IN, not =, "
        "since a name can match multiple real sub-department rows:\n"
        "    SELECT ... FROM trninvlabdet\n"
        "    WHERE DEPTCODE IN (SELECT SubDepartmentID FROM mstsubdepartment WHERE SubDeptName LIKE '%Haematology%')\n"
        ...
        "- CRITICAL, confirmed real bug: 'Radiology' LIKE-matches THREE mstsubdepartment rows — "
        "SubDepartmentID 33 ('RADIOLOGY PROCEDURE CHARGES'), 80 ('RADIOLOGY PROFESSIONAL CHARGES'), "
        "AND 78 ('Radiology Store', DEPARTMENTID=5 — an INVENTORY/STORE department, not diagnostic "
        "radiology at all). Including 78 in a 'radiology tests/procedures' query wrongly counts "
        "inventory-store activity as if it were diagnostic radiology. For radiology test/procedure "
        "counts, use ONLY 33 and 80 — exclude any sub-department whose name contains 'Store' even if "
        "it matched the LIKE search on the department word.",
        "- CRITICAL, confirmed real bug: a free-form 'completed radiology' query used generic "
        "TESTSTATUS='Result Entry' from trninvlabdet — bypassing the CONFIRMED radiology-specific "
        "completion signal (trninvstatus.STATUS='Authenticated', joined via BILLNO — see the "
        "get_lab_day_collection/get_department_dashboard notes elsewhere in this file). For "
        "'completed radiology' specifically, always check trninvstatus.STATUS='Authenticated' via "
        "BILLNO, not trninvlabdet.TESTSTATUS — that column doesn't reliably reflect radiology "
        "completion the way it does for lab departments.",
        ...
        "an 'average TAT' came back as 23,002,736 minutes (~44 years) — not a real answer. A bare "
        "'TAT'/'turnaround time' question has ALSO been confirmed to fail differently — schema search "
        "surfaced mstworkstations (an unrelated table, workstation IDs/names only) as a candidate and "
        "the question got abandoned there without ever reaching the right table. "
        "CORRECTION (was wrong before): trninvlabdet does NOT have a CREATEDATE column — confirmed "
        "via a real 'Invalid column name' error. The confirmed table+column pair for TAT is EXACTLY "
        "trnparamresult.BILLDATE (sample/order time) and trnparamresult.CREATEDATE (result time) — "
        "go straight there for any TAT question, don't rely on schema search alone to surface it, "
        "and don't try trninvlabdet.CREATEDATE, it doesn't exist. CONFIRMED real bug: a query once "
        "used trninvlabdet.REFUNDDATE as a stand-in 'result time' column instead — REFUNDDATE is "
        "about refund processing, not result completion, a semantically wrong substitute that still "
        "ran without erroring and produced a meaningless number presented as if it were real TAT. "
        "Never substitute a different date column just because it exists on the table and the query "
        "runs — TAT is SPECIFICALLY BILLDATE-to-CREATEDATE on trnparamresult, exactly those two "
        "columns, nothing else. Two things are BOTH required every "
        "time, not just when an error happens to occur — a real inconsistency already happened where "
        "one question applied both fixes correctly (reaching a real answer, 912 minutes) and a "
        "very similar question moments later skipped them and gave up on the same overflow error:\n"
        "  1. Bound the outlier range INSIDE the AVG's CASE WHEN (never as a WHERE clause — "
        "that would also wrongly shrink PROCEDURES/COMPLETED/PENDING counts elsewhere in the same "
        "query, undercounting real work rather than just fixing a skewed average).\n"
        "  2. CAST the DATEDIFF result to BIGINT before averaging — DATEDIFF(MINUTE, ...) can "
        "overflow SQL Server's INT range on a real outlier row and fail with an arithmetic overflow "
        "error; confirmed, real, happens on this exact data.\n"
        "The complete, correct formula, use this exactly every time, not a partial version:\n"
        "  AVG(CAST(CASE WHEN DATEDIFF(MINUTE, BILLDATE, CREATEDATE) BETWEEN 0 AND 10080 THEN "
        "DATEDIFF(MINUTE, BILLDATE, CREATEDATE) END AS BIGINT))\n"
        "The 10080 bound is 7 days in minutes, a generous upper limit for a real TAT; adjust if a "
        "narrower bound makes sense for the specific question, but never drop the CAST or the bound, "
        "and never report a raw unbounded/uncast AVG that could silently fail or be wrecked by bad "
        "data. A record with a bad date should still count as a "
