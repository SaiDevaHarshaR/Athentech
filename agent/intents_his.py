...
# ---------- pharmacy_sales_returns (real, from confirmed tblPharmSaleRetMst) ----------
def _handle_pharmacy_returns(q, role, db_name, db_server, db_user, db_password, matched_keyword=None):
    if role not in _ALLOWED_ROLES:
        return "Error: your role does not have access to this data."
    try:
        date_from, date_to, label = _period_dates(q)
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT COUNT(*) AS ReturnCount, SUM(NETAMT) AS TotalReturned
            FROM tblPharmSaleRetMst
            WHERE CAST(SALERETDT AS DATE) >= ? AND CAST(SALERETDT AS DATE) < ?
        """, (date_from, date_to))
        count, total = cursor.fetchone()
        if not count:
            return f"No pharmacy sales returns recorded ({label})."
        return _dashboard_card(
            icon="↩️", title="Pharmacy Sales Returns", subtitle=label,
            stats=[
                {"label": "RETURNS", "value": f"{int(count or 0):,}"},
                {"label": "TOTAL RETURNED", "value": f"₹{float(total or 0):,.0f}"},
            ],
        )
    except Exception as e:
        return f"Error: {e}"
...
# ---------- pharmacy_dept_issues (real, from confirmed tblPharmDeptIssueMst) ----------
def _handle_pharmacy_dept_issues(q, role, db_name, db_server, db_user, db_password, matched_keyword=None):
    if role not in _ALLOWED_ROLES:
        return "Error: your role does not have access to this data."
    try:
        date_from, date_to, label = _period_dates(q)
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT TOP 15 ISSUEFROMDEPT, ISSUETODEPT, NetAmt, ISSUEDT
            FROM tblPharmDeptIssueMst
            WHERE CAST(ISSUEDT AS DATE) >= ? AND CAST(ISSUEDT AS DATE) < ?
            ORDER BY ISSUEDT DESC
        """, (date_from, date_to))
        rows = cursor.fetchall()
        if not rows:
            return f"No pharmacy department issues recorded ({label})."
        return _list_card(
            icon="📤", title="Pharmacy Department Issues", intro=f"{label} — {len(rows)} shown:",
            items=[
                {"primary": f"{fromd or '-'} → {tod or '-'}", "fields": [f"₹{float(amt or 0):,.0f}", str(dt)]}
                for fromd, tod, amt, dt in rows
            ],
        )
    except Exception as e:
        return f"Error: {e}"
...
_INTENTS_HIS = [
    (["pharmacy returns", "sales returns", "pharmacy sales returns"], _handle_pharmacy_returns),
    (["pharmacy issues", "department issues", "dept issue", "pharmacy department issue"],
     _handle_pharmacy_dept_issues),
